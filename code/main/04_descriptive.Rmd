...
```{r save_plot_I}
png(here::here(figdir, "moran_I_blk_vil.png"), height = 1000, width = 2200, res = 250)
par(mfrow = c(1,2))
plot(bMC, xlab = "Block-level incidence", main = "")
plot(vMC, xlab = "Village-level incidence", main = "")
dev.off()
```
...
Calculate and map out ratio of village versus block incidence, overall and for a sample of blocks at different incidence levels.

```{r blk_vs_vil_ratio}

# # Map out the ratio of observed to expected
# v_shp_vl %>% 
#   mutate(exp = round(X2018_inc_blk),
#          obs_v_exp = X2018/exp,
#          obs_exp_diff = (X2018 - exp)) %>%
#   ggplot() +
#   geom_sf(aes(geometry = geometry, fill = obs_v_exp), col = NA) +
#   geom_sf(data = boundary, fill = NA) +
#   scale_fill_viridis_c(na.value = "white", direction = -1, option = "rocket") + 
#   labs(fill = "Observed - expected") -> obs_v_exp
# obs_v_exp
# 
# # ggsave(here::here("figures","obsvexp_vill_inc_2018.png"), height = 6, width = 8, dpi = 250)
# 
# # Look at observed vs expected for example high/mid/low incidence blocks 
# 
# blkdata <- v_shp_vl %>% 
#   dplyr::select(BLK_C_2011, X2018_inc_blk) %>% 
#   distinct() %>% 
#   mutate(inc_cat = cut(X2018_inc_blk*1e4, breaks = c(-Inf,0,0.5,1,2,Inf), right = TRUE))
# 
# summary(blkdata$inc_cat)
# # (-Inf,0]  (0,0.5]  (0.5,1]    (1,2] (2, Inf] 
# #    18940    18715     3698     2373     1068
# 
# blk_lo <- sample(blkdata$BLK_C_2011[blkdata$inc_cat == "(0,0.5]"],1)
# blk_lomid <- sample(blkdata$BLK_C_2011[blkdata$inc_cat == "(0.5,1]"],1)
# blk_himid <- sample(blkdata$BLK_C_2011[blkdata$inc_cat == "(1,2]"],1)
# blk_hi <- sample(blkdata$BLK_C_2011[blkdata$inc_cat == "(2, Inf]"],1)
# 
# samps <- c(`Low` = blk_lo, `Mid: under target` = blk_lomid, `Mid: above target` = blk_himid, `High` = blk_hi)
# 
# plot_block <- function(blk, nm){
#   vil_shp_vl_wblk %>% 
#     filter(dist_blk == blk) %>% 
#     ggplot() +
#     geom_sf(aes(geometry = geometry, fill = X2018_inc*1000), cex = 0.7, col = NA) +
#     # geom_sf(data = boundary, fill = NA) +
#     # scale_colour_viridis_c(trans = "identity", na.value = "white", direction = -1) + 
...
#     labs(fill = "Incidence\nper 1000",
#          title = nm
#          # title = "Expected village case counts by block level incidence"
#     ) -> p
#   return(p)
# }
# 
# plots <- purrr::map2(samps, names(samps), plot_block)
# gridExtra::grid.arrange(grobs = plots)
# ggsave(here::here("figures","expected_vill_inc_sampleblocks.png"), height = 12, width = 12)
